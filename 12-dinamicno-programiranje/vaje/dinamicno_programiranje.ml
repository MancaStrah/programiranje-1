...
let max_cheese cheese =
  ...
  let rec mouse x y = 
    ...
    cheese.(x).(y) + max right down 
  in
  mouse 0 0 
  
let max_cheese2 cheese = None
...
2) Združevanje rešitev (kako združimo rešitve delitve)

3) Robni pogoji (kako nehamo z delitvijo)
*)
...
let optimal_path cheese =
  ...
  let rec mouse x y = 
    ...
    if x = width - 1 && y = height - 1 then 
      ...
      let down_value = mouse x (y + 1) in 
      cheese.(y).(x) + down_value, Down :: down_path


    else if y = height then
      let right_value = mouse (x+1) y in 
      cheese.(y).(x) + right_value, Right :: right_path
