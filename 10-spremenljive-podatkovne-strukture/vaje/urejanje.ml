...
let min_and_rest' list =
	let rec aux min rest =  function 
		| [] -> Some (min, rest)
		| x :: xs -> (
			if x < min then aux x (min :: rest) xs else
			aux min (x :: rest) xs 
		)
		in 
		match list with 
...
(* let min_and_rest list = 
	let rec aux min rest list = 
		match list with 
		| [] -> Some (min, List.rev rest)
		| x :: xs -> (
			if x < min then aux x (min :: rest) xs else
			 aux min (x :: rest) xs
		)
...
(* Tale pa ne pomeša seznama rest, ampak samo odstrani najmanjši element. *)
let min_and_rest list =
	match list with
	| [] -> None
	| x :: xs -> (
		...
		let rec remove_min = function
			...
			| y :: ys (* when y <> min_x *) -> y :: (remove_min ys)
			in 
...
let rec selection_sort list =
	let rec selection_loop sorted_list list = 
		...
		| Some (m, rest) -> selection_loop (m :: sorted_list) rest
		| None -> List.rev sorted_list
	in 
	selection_loop [] list

(*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*]
 Urejanje z Izbiranjem na Tabelah (Inplace selection sort )
[*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*)
